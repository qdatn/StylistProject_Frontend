...
const OrderForm: React.FC<OrderFormProps> = ({
  ...
  const fetchAddressDetail = async (): Promise<void> => {
    try {
      const response = await axiosClient.getOne<Address>(
        `${baseUrl}/api/address/${order.address}`
      );
  ...
  const CancelOrder = async (order: Partial<Order>) => {
    ...
    if (order.status == "refunded" || order.status == "cancelled") {
      for (const item of order_items) {
        ...
        // 1. Tìm attribute tương ứng và tăng sl stock, giảm sl bán
      ...
      // 3. Cập nhật trạng thái đơn hàng là "cancelled"
      ...
      <div className="grid grid-cols-1 gap-x-12 gap-y-4 md:grid-cols-2 lg:grid-cols-2 pb-5">
        ...
        <div>
          ...
          <Select
            ...
            className={`w-full mt-1 h-10 rounded-md ${
      ...
      <div className="grid grid-cols-1 gap-x-12 gap-y-4 md:grid-cols-2 lg:grid-cols-2 pb-5">
        ...
        <div>
          ...
          <DatePicker
            ...
            className={`w-full mt-1 p-2 border rounded-md ${
              errors.receive_date ? "border-red-500" : ""
            }`}
          />
          {errors.receive_date && (
            <p className="text-red-500 text-sm">{errors.receive_date}</p>
          )}
        </div>
        <div>
