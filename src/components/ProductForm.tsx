...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const handleDeleteValue = (key: string, value: string) => {
    setAttributes((prev) =>
      prev.map((attr) =>
        attr.key === key
          ? { ...attr, value: attr.value.filter((v) => v !== value) }
          : attr
      )
    );
    setSelectedValues((prev) => {
  ...
  useEffect(() => {
    ...
    if (initialProduct?.categories) {
      const categoryIds = initialProduct.categories.map(
        (category) => (category as Category)._id
      );
    ...
    if (initialProduct?.attributes) {
      ...
      setAttributes((prevAttributes) => {
        return prevAttributes.map((attr) => {
          const productAttr = productAttributes.find(
            (pa) => pa.key === attr.key
          );
          return productAttr
            ? {
  ...
  const handleSave = () => {
    if (validate()) {
      ...
      const categoryIds = selectedCategoryObjects.map(
          ...
          {attributes.map((attr) => (
            <div key={attr.key} className="border-b border-gray-200 pb-4 mb-4">
              <div className="flex justify-between items-center">
                ...
                >
                  {attr.key}{" "}
                  {expandedKeys.has(attr.key) ? <IoRemove /> : <IoAddSharp />}
                </span>
                ...
                <div>
                  <div className="flex flex-wrap gap-4 mt-2">
                    {attr.value.map((val) => (
                      <div
                        key={val}
                        className="group flex items-center gap-2 bg-gray-100 px-2 py-1 rounded"
                      >
                        <input
                          type="checkbox"
          ...
          <div>
            ...
            {showNewCategoryForm ? (
              <div>
                ...
                <Button
                  className="text-[16px] p-4 w-full"
